...
void refresh_visual(int first_call, int progress, framebuffer_t *frame, zbuffer_t *z)
{
    packet_t *current;
    qword_t *q;
    qword_t *dmatag;
    ...
    if(first_call)
        create_local_screen(local_screen_fija, local_world, world_view, view_screen);
    
    // Calculate the vertex values.
    calculate_vertices(temp_vertices, vertex_count, vertices, local_screen);
    calculate_vertices(temp_vertices_position, 12, vertices_position, local_screen);
    calculate_vertices(temp_vertices_fondo, vertex_count, vertices_fondo, local_screen);
    calculate_vertices(temp_vertices_avance, 120, vertices_avance, local_screen_fija);
    
    // Convert floating point vertices to fixed point and translate to center of screen.
    draw_convert_xyz(verts, 2048, 2048, 32, vertex_count, (vertex_f_t*)temp_vertices);
    draw_convert_xyz(verts_position, 2048, 2048, 32, 12, (vertex_f_t*)temp_vertices_position);
    draw_convert_xyz(verts_fondo, 2048, 2048, 32, vertex_count, (vertex_f_t*)temp_vertices_fondo);
    draw_convert_xyz(verts_avance, 2048, 2048, 32, 120, (vertex_f_t*)temp_vertices_avance);
    
    // Convert floating point colours to fixed point.
    draw_convert_rgbq(colors, vertex_count, (vertex_f_t*)temp_vertices, (color_f_t*)colors_rgb, color.a);
    draw_convert_rgbq(colors_position, 12, (vertex_f_t*)temp_vertices_position, (color_f_t*)colors_position_rgb, color.a);
    draw_convert_rgbq(colors_fondo, vertex_count, (vertex_f_t*)temp_vertices_fondo, (color_f_t*)colors_fondo_rgb, color.a);
    draw_convert_rgbq(colors_avance, 120, (vertex_f_t*)temp_vertices_avance, (color_f_t*)colors_avance_rgb, color.a);
...
void setear_color()
{
    ...
    for(i=0;i<54;i++) {    
        switch(i/9) {
            case 0: cambiar_color(i,0);break;
            case 1: cambiar_color(i,1);break;
            case 2: cambiar_color(i,2);break;
            case 3: cambiar_color(i,3);break;
            case 4: cambiar_color(i,4);break;
            case 5: cambiar_color(i,5);break;
        }        
    }
...
void cambiar_cuadrado(int j,int k)
{
    ...
    for(l=0;l<4;l++){
        for(i=0;i<3;i++){ 
            ...
            vertices_position[l*3+1][i]=vertices[j*36+k*4+(l+1)%4][i];
            vertices_position[l*3+2][i]=vertices[j*36+k*4+(l+2)%4][i];
        }    
    }
