...
int rubik(framebuffer_t *frame, zbuffer_t *z)
{
    ...
    while(1)
    {
                ...
                if(st.movement.state_move == mov_none
                   && st.coloreo_inicial_finalizado==YES) {
                    ...
                    switch(st.cursor.cara_actual) {
                        case 0 : mov=mov_R; break;
                        case 1 : mov=mov_L; break;
                        case 2 : mov=mov_F; break;
                        case 3 : mov=mov_B; break;
                        case 4 : mov=mov_U; break;
                        case 5 : mov=mov_D; break;
                    }
                ...
                if(st.coloreo_inicial_finalizado==NO)
                    ...
                    if (st.movement.move_count==0){
                        if(st.cambio_en_solucion==YES){
                            ...
                            st.movement.state_move=st.sol_reader.solucion[st.sol_reader.mov_index];
                            if(st.sol_reader.solucion[st.sol_reader.mov_index] != 0) st.sol_reader.mov_index++;
                        }
                    }
                }
            }
        ...
        if(st.solver.state!=empty_state) {
            switch(st.solver.state) {
                ...
                case solution_top: {
                    ...
                    st.solver.cubo_ptr=rubik_solver(st.solver.cubo_ptr,st.solver.etapa,st.solver.rotacion);
                ...
                case solution_low: {
                    ...
                    st.solver.cubo_ptr=beginner_solver(st.solver.cubo_ptr,st.solver.etapa);
        ...
        if(st.movement.state_move!=mov_none && clock()-timer_principal>TIME_MOVE) {
        ...
        switch(st.solver.state) {
            case empty_state: progress=100;break;
            case solution_top: progress=(st.solver.rotacion-1)*17+st.solver.etapa*3;break;
            case solution_med: progress=st.solver.etapa*20;break;
            case solution_low: progress=st.solver.etapa*11;break;
            default: break;
        }
